    ...
    <div class="sidebar-content">
        ...
        <div class="sidebar-menu">
            <ul>
                ...
                @canany(['ads.view', 'ads.create', 'ads.edit', 'ads.delete', 'ads.schedule', 'screens.view',
                    'screens.create', 'screens.edit', 'screens.delete', 'places.view', 'places.create', 'places.edit',
                    'places.delete', 'monitoring.view', 'monitoring.manage', 'reports.view', 'reports.generate',
                    'logs.view', 'logs.export'])
                    @php($adsSystemNav = nav_active(['admin.ads.*', 'admin.screens.*', 'admin.places.*', 'admin.monitoring.*', 'admin.reports.*', 'admin.logs.*']))
                    <li @class([
                        'sidebar-dropdown',
                        'active' => $adsSystemNav['is_active'],
                        'open' => $adsSystemNav['is_active'],
                    ])>
                ...
                @canany(['admins.view', 'admins.create', 'admins.edit', 'admins.delete', 'permissions.view',
                    'roles.view'])
                    @php($adminManagementNav = nav_active(['admin.admins.*', 'admin.permissions.*', 'admin.roles.*']))
                    <li @class([
                        'sidebar-dropdown',
                        'active' => $adminManagementNav['is_active'],
                        'open' => $adminManagementNav['is_active'],
                    ])>
                ...
                @can('users.view')
                    ...
                    <li @class([
                        'sidebar-dropdown',
                        'active' => $usersNav['is_active'],
                        'open' => $usersNav['is_active'],
                    ])>
                ...
                @can('cms.manage')
                    @php($cmsNav = nav_active(['admin.cms.*', 'admin.cms.pages.*', 'admin.seo_metas.*']))
                    <li @class([
                        'sidebar-dropdown',
                        'active' => $cmsNav['is_active'],
                        'open' => $cmsNav['is_active'],
                    ])>
                        ...
                        <div @class(['sidebar-submenu', 'open' => $cmsNav['is_active']])>
                            <ul>
                                @php($cmsHomeNav = nav_active('admin.cms.seo_metas.*'))
                                <li @class(['active' => $cmsHomeNav['is_active']])>
                                    <a href="{{ route('admin.seo_metas.index', ['lang' => $locale]) }}"
                                        class="{{ Request::is('dashboard/seo_metas') ? 'current-page' : '' }}">
                                        {{ __('admin.sidebar.seo_metas') }}
                                    </a>
                                </li>
                ...
                @canany(['contact_submissions.view', 'contact_submissions.delete'])
                    ...
                    <li @class([
                        'sidebar-dropdown',
                        'active' => $contactSubmissionsNav['is_active'],
                        'open' => $contactSubmissionsNav['is_active'],
                    ])>
